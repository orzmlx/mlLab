...
In this assignment,we will chose different values of M,and compare compare their fit results.
...
for(it in 1:max_it) {
  ...
  points(mu[3,], type="o", col="green")#This line should ignored if K=2 
  #points(mu[4,], type="o", col="yellow")#This line should ignored if K=2 or k=3
  Sys.sleep(0.5)
...
We can get results for different K values.According to the mu value, when M=3, the model fits the true structure of the data well (the data is generated from 3 components), so the clustering and mean estimates are close to the true value.
When M=4, mu[1,] and mu[2,] are close to the true value mu[3,], which may have the risk of overfitting; when M=2, the true distribution of the data (3 clusters) cannot be well matched, which may have the risk of underfitting.
