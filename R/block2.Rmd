...
<<<<<<< HEAD
#build the train dataset for 1000 times with the size of 100 Create 1000
training data sets of size 100, learn a random forest from each data
set, and compute the classification error in the same test data set of
size 1000. Report results for when the random forest has 1, 10 and 100
trees.
...
each data set, and compute the classification error in the same test
data set of size 1000. Report results for when the random forest has 1,
10 and 100 trees.
>>>>>>> 8160302e365851763e12202687abf1e0dfc0db96
```{r train dataset}
set.seed(123)
...
We can get results for different K values.When M is equal to 2, the
